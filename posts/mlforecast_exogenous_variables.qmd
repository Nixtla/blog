...
# DATA_URL = 'https://raw.githubusercontent.com/Nixtla/nixtla_blog/main/posts/data/mlforecast_exogenous/store_sales_subset.csv'
...
```text
Static features MAE: 615.56 (-0.2% improvement over baseline)
```

Static features alone don't improve accuracy here since the baseline already captures series-specific patterns through lag features. The value of static features becomes apparent when combined with other exogenous variables.
...
Dynamic features MAE: 607.79 (1.0% improvement over baseline)

Promotions and oil prices provide modest gains. In retail, promotion effects are often already reflected in the target variable's patterns, while oil price impacts consumer behavior indirectly over longer time horizons.
...
```text
Calendar features MAE: 591.17 (3.7% improvement over baseline)
```

Calendar features outperform all other exogenous variables. Unlike oil prices with gaps or promotion counts that vary unpredictably, day-of-week and month contain no noise or missing values. The model learns from these perfectly reliable signals consistently.
