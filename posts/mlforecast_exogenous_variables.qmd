...
Time series forecasting rarely depends on historical values alone. Sales are influenced by prices, promotions affect demand, and holidays create predictable spikes. Incorporating these external factors can dramatically improve forecast accuracy.

The challenge is that different types of external data require different handling:

- **Static features** like store location or product category never change
- **Dynamic features** like prices or promotions change over time and are known in advance
- **Calendar patterns** like day-of-week effects repeat predictably

Traditional approaches require manual feature engineering to handle each type correctly. You need to replicate static features across timestamps, align future values with prediction horizons, and extract calendar attributes without data leakage.
...
MLForecast distinguishes between two categories of exogenous features based on how they behave over time:

**Static features** remain constant for each time series. Store metadata, product categories, and geographic regions fall into this category. MLForecast replicates these values automatically when generating predictions.

**Dynamic features** change at each timestamp. Prices, promotional flags, and weather forecasts are dynamic. Since these values are known in advance (you set next week's prices today), you provide them at prediction time through the `X_df` parameter.

The workflow is straightforward:

1. Merge your time series with exogenous data
2. Specify static columns in the `fit()` call
3. Provide future dynamic values in the `predict()` call

All columns not marked as static are treated as dynamic exogenous variables by default.
...
0          0 2000-01-01  17.519033         4          82
1          0 2000-01-02   0.276729         4          82
2          0 2000-01-03   6.617829         4          82
3          0 2000-01-04   2.844439         4          82
4          0 2000-01-05   7.249720         4          82
```

The data contains 100 time series, each with two static features (`static_0` and `product_id`) that remain constant across all timestamps for a given series.
...
fcst.fit(series, static_features=['static_0', 'product_id'])
```
...
The model now uses `static_0` and `product_id` as features alongside the generated lag features. During prediction, these static values are automatically carried forward:
...
```text
['static_0', 'product_id', 'price', 'lag7', 'expanding_mean_lag1', 'rolling_mean_lag7_window_size14']
```
...
```text
['static_0', 'product_id', 'lag7', 'dayofweek', 'month', 'is_weekend']
```
...
```text
   unique_id         ds          y  static_0  product_id  sin1_7    cos1_7    sin2_7    cos2_7
